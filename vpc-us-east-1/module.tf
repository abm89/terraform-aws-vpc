...
resource "aws_internet_gateway" "default" {
  vpc_id = "${aws_vpc.main-us-east.id}"
...
resource "aws_subnet" "public" {
  ...
  vpc_id                          = "${aws_vpc.main-us-east.id}"
  cidr_block                      = "${cidrsubnet(aws_vpc.main-us-east.cidr_block, 6, count.index)}"
...
resource "aws_subnet" "private" {
  count                           = "${length(data.aws_availability_zones.available.names)}"
  vpc_id                          = "${aws_vpc.main-us-east.id}"
  cidr_block                      = "${cidrsubnet(aws_vpc.main-us-east.cidr_block, 6, count.index + aws_subnet.public.count)}"
  availability_zone               = "${element(data.aws_availability_zones.available.names, count.index)}"
...
#  ami                    = "ami-01a76e79ae757048d"
#  subnet_id              = "${element(aws_subnet.public.*.id, count.index)}"
#  vpc_security_group_ids = ["${aws_security_group.default.id}", "${aws_vpc.main-us-east.default_security_group_id}"]
#
#  tags = {
